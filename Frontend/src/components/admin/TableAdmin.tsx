...
interface TableAdminProps {
  ...
  onCreate?: () => void;
  actions?: (item: any, refresh: () => Promise<void>) => JSX.Element | null;
}
...
export const TableAdmin: React.FC<TableAdminProps> = ({
  ...
  const fetchData = async () => {
    try {
      ...
      // Intentar mezclar con mocks guardados en localStorage (modo demo)
      try {
        const storageKey = `mock_${tableName.replace(/\s+/g, '_').toLowerCase()}`; // e.g. 'mock_empresas'
        const raw = localStorage.getItem(storageKey) || '[]';
        const localMocks = JSON.parse(raw);
        // Evitar duplicados por primaryKey: los del backend tienen prioridad
        const map = new Map<any, any>();
        (response.data || []).forEach((item: any) => map.set(item[primaryKey], item));
        (localMocks || []).forEach((item: any) => {
          if (!map.has(item[primaryKey])) map.set(item[primaryKey], item);
        });
        setData(Array.from(map.values()));
      } catch (e) {
        // Si algo falla con localStorage, solo usar la respuesta del backend
        setData(response.data);
      }
    } catch (error) {
      console.error(`Error fetching ${tableName}:`, error);
      // Si el backend falla, cargar desde localStorage como fallback
      try {
        const storageKey = `mock_${tableName.replace(/\s+/g, '_').toLowerCase()}`;
        const raw = localStorage.getItem(storageKey) || '[]';
        const localMocks = JSON.parse(raw);
        setData(localMocks || []);
      } catch (e) {
            ...
            {filteredData.map((item) => (
              <tr key={item[primaryKey]} className="hover:bg-yellow-100">
                ...
                <td className="px-4 py-2 border border-yellow-400 space-x-2 flex">
                  {actions && actions(item, fetchData)}
                  {onEdit && (
                    <button
