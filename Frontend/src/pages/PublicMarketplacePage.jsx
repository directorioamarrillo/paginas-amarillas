...
function ProductCard({ producto }) {
  ...
  return (
    <div
      className="group cursor-pointer rounded-2xl border border-brand-gray-light bg-white shadow-[0_2px_10px_rgba(0,0,0,0.02)] transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
    ...
    >
      <div className="relative overflow-hidden rounded-t-2xl bg-gradient-to-br from-slate-100 to-slate-50 aspect-square">
        ...
        {/* Category Badge */}
        {categoriaNombre && (
          <div className="absolute bottom-3 left-3 rounded-full bg-white/95 px-3 py-1.5 text-xs font-semibold text-brand-gray shadow-md backdrop-blur">
            {categoriaNombre}
          </div>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        {/* Product Name */}
        <h3 className="line-clamp-2 text-sm font-semibold text-brand-dark group-hover:text-primary-dark transition-colors duration-200">
          {producto.nombre}
        </h3>

        {/* Price */}
        <div className="mt-3 flex items-baseline gap-2">
          <span className="text-2xl font-bold text-brand-dark">{precioFormateado}</span>
        </div>
        ...
        <div className="mt-3 flex items-center justify-between">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-brand-gray-light/20 px-2.5 py-1 text-xs font-medium text-brand-gray">
      ...
      <div className="border-b border-brand-gray-light bg-white shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
        <div className="mx-auto max-w-7xl px-4 py-4 md:px-6">
          <div className="flex items-center gap-3">
            <Link to="/" className="text-sm font-semibold text-primary-dark hover:text-primary-dark">
              Inicio
            </Link>
            <span className="text-brand-gray-light">/</span>
            <span className="text-sm font-semibold text-brand-gray">Marketplace</span>
          </div>
          ...
          <form onSubmit={handleSearch} className="mt-3">
            <div className="flex gap-2">
              <div className="relative flex-1">
                <FontAwesomeIcon
                  icon={faSearch}
                  className="absolute top-1/2 left-4 -translate-y-1/2 text-brand-gray-light"
                />
                <input
                  type="text"
                  value={search}
                  ...
                  className="w-full rounded-xl border border-brand-gray-light py-3 pl-12 pr-4 text-brand-dark placeholder:text-brand-gray-light focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500"
                />
              </div>
              <button
                type="submit"
                className="rounded-xl bg-primary px-6 py-3 font-semibold text-brand-dark transition hover:bg-primary-400"
              >
                Buscar
              </button>
            </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 md:px-6">
        <div className="flex gap-6">
          ...
          <aside className="hidden w-64 shrink-0 lg:block">
            <div className="sticky top-6 space-y-4 rounded-xl border border-brand-gray-light bg-white p-5 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
              ...
              {/* Category Filter */}
              <div>
                <label className="mb-2 block text-sm font-semibold text-brand-gray">Categoría</label>
                <Select
              ...
              {/* Company Filter */}
              <div>
                <label className="mb-2 block text-sm font-semibold text-brand-gray">Empresa</label>
                <Select
              ...
              <button
                ...
                className="w-full rounded-lg bg-primary py-2.5 font-semibold text-brand-dark transition hover:bg-primary-400"
              >
                Aplicar filtros
              </button>
            </div>
          ...
          <main className="min-w-0 flex-1">
            ...
            <div className="mb-4 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex items-center gap-2 rounded-lg border border-brand-gray-light bg-white px-4 py-2 text-sm font-semibold text-brand-gray hover:bg-brand-gray-light/10 lg:hidden"
                >
                  <FontAwesomeIcon icon={faFilter} />
                  Filtros
                  {activeFiltersCount > 0 && (
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-primary text-xs font-bold text-brand-dark">
                      {activeFiltersCount}
                    </span>
                  )}
                </button>
                <p className="text-sm text-brand-gray">
                  <span className="font-semibold text-brand-dark">{formatCompact(productos.data?.length || 0)}</span>{" "}
              ...
              <select
                ...
                className="rounded-lg border border-brand-gray-light px-3 py-2 text-sm font-medium focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500"
              >
            ...
            {/* Mobile Filters */}
            {showFilters && (
              <div className="mb-4 rounded-xl border border-brand-gray-light bg-white p-4 lg:hidden">
                <div className="mb-3 flex items-center justify-between">
                  <h3 className="font-semibold text-brand-dark">Filtros</h3>
                  <button onClick={() => setShowFilters(false)}>
                    <FontAwesomeIcon icon={faXmark} className="text-brand-gray-light" />
                  </button>
                </div>

                <div className="space-y-3">
                  <div>
                    <label className="mb-1 block text-xs font-semibold text-brand-gray">Categoría</label>
                    <select
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-semibold text-brand-gray">Precio</label>
                    <div className="flex gap-2">
                      <input
                        type="number"
                        placeholder="Mín"
                  ...
                  <button
                    ...
                    className="w-full rounded-lg bg-primary py-2.5 font-semibold text-brand-dark"
                  >
                    Aplicar
                  </button>
                </div>
