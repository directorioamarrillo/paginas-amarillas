...
export const ReviewForm: React.FC<ReviewFormProps> = ({ id_review }) => {
  ...
  const fetchReview = async (): Promise<void> => {
    try {
      const response = await api.get(`/api/reviews/${id_review}`);
      ...
      // Fallback: backend endpoint returns reviews by id_empresa, not id_review
      const listResp = await api.get<Review[]>(`/api/reviews/?skip=0&limit=1000`);
      const found = (listResp.data as any[]).find((r) => r.id_review === Number(id_review));
      if (found) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      if (id_review) {
        await api.put(`/api/reviews/${id_review}`, data);
      } else {
