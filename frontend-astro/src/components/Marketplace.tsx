...
export function Marketplace({ rol = "user", productos: initialProductos = [] }: { rol?: string; productos?: Producto[] }) {
  ...
  useEffect(() => {
    // If produtos were passed from server as props, skip fetching
    if (initialProductos && initialProductos.length > 0) return;
    setLoading(true);
    axiosInstance.get("/api/marketplace")
  ...
  const productosFiltrados = useMemo(() =>
    productos.filter(p => {
      ...
      return (
        ...
        precio >= Number(minPrecio) &&
        precio <= Number(maxPrecio)
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
  const res = await axiosInstance.post("/api/marketplace", form);
  ...
  const handleDelete = async (id: number) => {
    ...
    setError("");
    try {
  await axiosInstance.delete(`/api/marketplace/${id}`);
            ...
            productosFiltrados.map((p) => (
              <div key={p.id_marketplace} className="bg-white rounded-lg shadow-md p-4 flex flex-col items-center">
                {p.imagen_url ? (
                  ...
                  <div className="w-32 h-32 bg-gray-200 rounded mb-4 flex items-center justify-center text-gray-500">Sin imagen</div>
                )}
